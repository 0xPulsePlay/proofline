...
# packages/config/deployments/base-sepolia.json.
#
...
BROADCAST="$REPO_ROOT/contracts/base/broadcast/Deploy.s.sol/84532/run-latest.json"
OUT_DIR="$REPO_ROOT/packages/config/deployments"
mkdir -p "$OUT_DIR"

node - "$BROADCAST" "$OUT_DIR/base-sepolia.json" "$REPO_ROOT" <<'EOF'
const fs = require("fs");
